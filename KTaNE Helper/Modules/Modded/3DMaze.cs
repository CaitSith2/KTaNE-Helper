...
namespace KTaNE_Helper.Modules.Modded
{
    public class _3DMaze
    {
        ...
        {
            return GetLocationList(line.Split(new[] {Environment.NewLine}, StringSplitOptions.RemoveEmptyEntries), maze);
        }

        public List<int[]> GetLocationList(string[] lines, string maze)
        {
            List<int[]> prevLocation = new List<int[]>();
            foreach (var line in lines)
            {
                var m = GetLocationList(line, maze, prevLocation);
                prevLocation = m;
            }
            return prevLocation;
        }

        public bool IsLocationIn(int current, int prev, int length, bool moveNorthWest)
        {
            for (var i = 0; i < length; i++)
            {
                if (current == prev)
                    return true;
                current += moveNorthWest ? 7 : 1;
                current %= 8;
            }
            return false;
        }

        public List<int[]> GetLocationList(string line, string maze, List<int[]> prevLocations)
        {
            ...
            do
            {
                ...
                for (var i = 0; i < 8; i++)
                {
                    ...
                    do
                    {
                        ...
                        {
                            var lineEndsAtPrevious = false;
                            foreach (var prev in prevLocations)
                            {
                                if (prev[0] == (rows ? i : rj))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? rj : i, prev[1], prev[4], true);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? rj : i, prev[1], prev[4], false);
                                    }
                                }
                                else if (prev[1] == (rows ? rj : i))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : rj, prev[0], prev[4], true);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : rj, prev[0], prev[4], false);
                                    }
                                }

                                else if (prev[0] == (rows ? i : oj))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? oj : i, prev[1], prev[4], true);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? oj : i, prev[1], prev[4], false);
                                    }
                                }
                                else if (prev[1] == (rows ? oj : i))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : oj, prev[0], prev[4], true);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : oj, prev[0], prev[4], false);
                                    }
                                }

                                /*if(prev[0] == (rows ? i : rj) || prev[1] == (rows ? rj : i))
                                {
                                    lineEndsAtPrevious |= prev[2] == (rows ? 2 : 1) || prev[2] == (rows ? 0 : 3);
                                }
                                if (prev[0] == (rows ? i : oj) || prev[1] == (rows ? oj : i))
                                {
                                    lineEndsAtPrevious |= prev[2] == (rows ? 2 : 1) || prev[2] == (rows ? 0 : 3);
                                }*/
                            }
                            lineEndsAtPrevious |= prevLocations.Count == 0;

                            if (lineEndsAtPrevious)
                                locations.Add(
                                new[]
                                {
                                    rows ? i : rj,
                        ...
                        if (line.Replace('*', ' ').ToUpper() == mazeline.Reverse().Replace('*', ' '))
                        {
                            ...
                            foreach (var prev in prevLocations)
                            {
                                if (prev[0] == (rows ? i : rj))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? rj : i, prev[1], prev[4], rows);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? rj : i, prev[1], prev[4], !rows);
                                ...
                                else if (prev[1] == (rows ? rj : i))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : rj, prev[0], prev[4], rows);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : rj, prev[0], prev[4], !rows);
                                ...
                                else if (prev[0] == (rows ? i : oj))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? oj: i, prev[1], prev[4], rows);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? oj : i, prev[1], prev[4], !rows);
                                ...
                                else if (prev[1] == (rows ? oj : i))
                                {
                                    if (prev[2] == (rows ? 2 : 1))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : oj, prev[0], prev[4], rows);
                                    }
                                    else if (prev[2] == (rows ? 0 : 3))
                                    {
                                        lineEndsAtPrevious |= IsLocationIn(rows ? i : oj, prev[0], prev[4], !rows);
        ...
        {
            return FindLocation(line.Split(new[] {Environment.NewLine}, StringSplitOptions.RemoveEmptyEntries), maze);
        }

        public string FindLocation(string[] lines, string maze)
        {
            var result = "";
            maze = maze.ToUpperInvariant();
            if (!Mazes.Contains(maze))
            {
                var locationsFound = false;
                foreach (var x in Mazes.Where(y => maze.Aggregate(true, (current, c) => current & y.Contains(c))))
                {
                    var y = FindLocation(lines, x);
                    if (y == "Location not found") continue;
                    locationsFound = true;
                    if (y == "Too Many Locations possible")
                    {
                        result += "Too Many Locations possible in maze " + x + Environment.NewLine;
                        continue;
                    }
                    result += "In maze " + x + ":" + Environment.NewLine;
                    result += y + Environment.NewLine;
                }
                return !locationsFound ? "Location not found" : result;
            }
            ...
            if (lines == null || lines.Length == 0) return "Location not found";
            foreach (var line in lines)
            {
                var m = GetLocationList(line, maze, prevLocation);
                if (m.Count == 0) return "Location not found";
                prevLocation = m;
            }

            locations = prevLocation.Select(l => "Row: " + l[0] + ", Column: " + l[1] + walls[l[2]] + cardinal[l[3]]).ToList();
        ...
        public string[] Mazes = {"ABC", "ABD", "ABH", "ACD", "ACH", "ADH", "BCD", "BCH", "BDH", "CDH"};
